...
function initDaemon(cb) {
	exec('aria2c --enable-rpc --daemon --quiet', (err) => {
		if (err !== null) {
	...
	aria.request(func, args, (err, res)=>{
		if(err) {
			...
			if(err.code && err.code === 'ECONNREFUSED') {
				...
				if(retries<=0) {					
					retries++
					return initDaemon(()=>request(func, args, cb))
				}
			}
			// tried or else
			console.error(err)
...
exports.shutdown = () => {
	return request('aria2.shutdown',[],(res)=>{console.log(res.result)})
}

exports.forceShutdown = () => {
	return request('aria2.forceShutdown',[],(res)=>{console.log(res.result)})
}
